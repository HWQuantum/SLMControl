...
def analyse_first_set():
    def plot_data(filename, axis):
        ...
        print((data[0][38] + data[2][88] - data[2][38] - data[0][88]) /
              (data[0][38] + data[2][88] + data[2][38] +
               data[0][88]))  # - E(A, Bd)
        ...
        def normalisation(a, b):
            ...
            for x in [0, np.pi / 2]:
                for y in [0, np.pi / 2]:
                    total += data[closest_index(a + x,
                                                angles_a)][closest_index(
                                                    b + y, angles_b)]
            return total
    ...
    E1 = (sins[0](vb) + sins[2](vb + np.pi) - sins[2](vb) -
          sins[0](vb + np.pi)) / (sins[0](vb) + sins[2](vb + np.pi) +
                                  sins[2](vb) + sins[0](vb + np.pi))
    ...
    E3 = (sins[1](vb) + sins[3](vb + np.pi) - sins[3](vb) -
          sins[1](vb + np.pi)) / (sins[1](vb) + sins[3](vb + np.pi) +
                                  sins[3](vb) + sins[1](vb + np.pi))
...
def analyse_second_set():
    with open('./2x2_1_to_5_bell_data', 'rb') as f:
        d = pickle.load(f)

        for l in [1, 2, 3, 4, 5]:
            a = np.arange(0, 2 * np.pi, np.pi / 2)
            b = np.arange(np.pi / 4, 2 * np.pi, np.pi / 2)

            def c(a, b):
                return d[('l_a_b', l, a, b)][0][3][3]

            E1 = abs((c(a[0], b[0]) + c(a[2], b[2]) - c(a[2], b[0]) -
                  c(a[0], b[2])) / (c(a[0], b[0]) + c(a[2], b[2]) +
                                    c(a[2], b[0]) + c(a[0], b[2])))

            E2 = abs((c(a[0], b[1]) + c(a[2], b[3]) - c(a[2], b[1]) -
                  c(a[0], b[3])) / (c(a[0], b[1]) + c(a[2], b[3]) +
                                    c(a[2], b[1]) + c(a[0], b[3])))

            E3 = abs((c(a[1], b[0]) + c(a[3], b[2]) - c(a[3], b[0]) -
                  c(a[1], b[2])) / (c(a[1], b[0]) + c(a[3], b[2]) +
                                    c(a[3], b[0]) + c(a[1], b[2])))

            E4 = abs((c(a[1], b[1]) + c(a[3], b[3]) - c(a[3], b[1]) -
                  c(a[1], b[3])) / (c(a[1], b[1]) + c(a[3], b[3]) +
                                    c(a[3], b[1]) + c(a[1], b[3])))

            print("In l={}\nE1={} E2={} E3={} E4={}\nS={}".format(l, E1, E2, E3, E4, E1+E2+E3+E4))
